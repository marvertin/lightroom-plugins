                ...
                while not shutdown and not self.closed do
                    ...
                    if s ~= nil then
                        ...
                        if p1 then
                            ...
                            resp = resp:sub( p2 + 1 ) -- "" -- Added 2/Jul/2012 6:46 so response returned does not keep growing: not sure how this ever worked before without it ###3.
...
function Session:_close()
    ...
    if self.cmdfile ~= nil then    
        ...
        self.args = {}
        self.targs = {}
        self.closed = true -- why not just have cmdfile act as "closed flag"? ###3 - maybe there was a reason...
        -- not sure why cmdfile is not being nil'd, but best not to nil args & targs.
