...
function Disk:existsAs( path, type, throwErrorIfWrongType )
    local exists = LrFileUtils.exists( path ) -- ###1 what if path is nil or blank?
    if exists then
        if exists == type then
...
--
function Disk:deleteTree( tree, trash, leaveRoot )
...
Disk.assureDirectories = Disk.assureAllDirectories
Disk.assureDirectory = Disk.assureDirectories
Disk.assureDir = Disk.assureDirectory
