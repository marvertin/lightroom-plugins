...
function App:executeCommand( command, parameters, targets, output, handling )
    ...
    return self.os:executeCommand( command, _parameters, _targets, output, handling )
end
...
App.logVerb = App.logVerbose -- terse synonym 
App.logv = App.logVerbose -- terse synonym 
...
App.logw = App.logWarning -- synonym
...
function App:sleepUnlessShutdown( time, incr, doneFunc )
    ...
    while not shutdown and (LrDate.currentTime() - startTime < time) do
        LrTasks.sleep( incr )
        if doneFunc and doneFunc() then
            return
        end
    end
    -- nothing returned - check shutdown flag in calling context, if desired.
end
App.sleep = App.sleepUnlessShutdown -- shorter synonym.
